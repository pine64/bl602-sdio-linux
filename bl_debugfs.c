...
extern unsigned long bl_filter_severity;
extern unsigned long bl_filter_module;

static ssize_t bl_dbgfs_flt_severity_read(struct file *file,
                                       char __user *user_buf,
                                       size_t count, loff_t *ppos)
{
    char buf[32];
	int ret;
	ssize_t read;
    ret = scnprintf(buf, sizeof(unsigned long), "%lx\n", bl_filter_severity);
    read = simple_read_from_buffer(user_buf, count, ppos, buf, ret);

    return read;
}
static ssize_t bl_dbgfs_flt_severity_write(struct file *file,
                                        const char __user *user_buf,
                                        size_t count, loff_t *ppos)
{
    char buf[32];
...
DEBUGFS_READ_WRITE_FILE_OPS(flt_severity);
static ssize_t bl_dbgfs_flt_module_read(struct file *file,
                                       char __user *user_buf,
                                       size_t count, loff_t *ppos)
{
	int ret;
...
static ssize_t bl_dbgfs_flt_module_write(struct file *file,
                                        const char __user *user_buf,
                                        size_t count, loff_t *ppos)
{
    char buf[32];
...
DEBUGFS_READ_WRITE_FILE_OPS(flt_module);
...
int bl_dbgfs_register(struct bl_hw *bl_hw, const char *name)
{
    ...
    DEBUGFS_ADD_FILE(flt_severity, dir_drv, S_IWUSR | S_IRUSR);
    DEBUGFS_ADD_FILE(flt_module, dir_drv, S_IWUSR | S_IRUSR);
