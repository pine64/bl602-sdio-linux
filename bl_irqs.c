...
{    
	...
	list_for_each_entry(cur, &bl_hw->cmd_mgr.cmds, list) {
			...
			if(cur->tkn == hostid->tkn) {
				...
				bl_write_data_sync(bl_hw, (u8 *)(cur->a2e_msg), sizeof(struct lmac_msg) + cur->a2e_msg->param_len, bl_hw->plat->io_port + CTRL_PORT);
				kfree(cur->a2e_msg);
				break;
			}
	}
...
static int bl_upload_hdl(struct bl_hw *bl_hw)
{
    ...
    u32 rd_bitmap = 0;
    bool rd_twice = false;
    __kernel_suseconds_t start;
    __kernel_suseconds_t end;
...
TWICE:
	...
	while(true) {
		...
		if(rx_len > BL_SDIO_BLOCK_SIZE) {
			rx_blocks = DIV_ROUND_UP(rx_len, BL_SDIO_BLOCK_SIZE);
			rx_len = rx_blocks * BL_SDIO_BLOCK_SIZE;
		}
